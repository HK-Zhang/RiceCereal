testHtmlRegEx = (str) => {
    const cssEscapedRegEx = /(?<=id=")[^"]+|(?<=id=')[^']+|(?<=\[id\]=")[^"]+|(?<=\[id\]=')[^']+|(?<=<)[\w_-]+|(?<=[\[?ng]{0,2}class[Name]{0,4}\]?=")[^"]+|(?<=[\[?ng]{0,2}class[Name]{0,4}\]?=')[^']+|(?<=@include\s)[^\s]+|(?<=\bstyles.)[\w_-]+|(?<=[\[?ng]{0,2}class[Name]{0,4}\]?=\{`).+(?=\`)/mgi;
    const escapedClass = [];
    let match;
  ...
  test('              <div className={`mainContainer  as`}>', () => {
    expect(testHtmlRegEx('              <div className={`mainContainer  as`}>')).toContain("mainContainer  as");
  });